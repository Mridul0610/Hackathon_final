...
function StuHome() {
  ...
  const isMentorLoggedIn = sessionStorage.getItem('studentId') !== null;
  if (!isMentorLoggedIn) {
  ...
  let token={
    mentorsId:sessionStorage.getItem("studentId"),
  }
...
{items.filter(item=> item.status===1).map(item=>(
          ...
          <div className="card" data-aos="fade-up" data-aos-delay={100} style={{ boxShadow: '0 4px 8px 0 rgba(0,0,0,0.2)', borderRadius: '5px', overflow: 'hidden',lineHeight:0 ,height:'auto'}}>
            <div className="card-body">
              ...
              <h5 style={{color:'white'}}><span className="badge badge-info" style={{background:'white', color:'black'}}>Available slots</span>  {item.availableSlots}</h5>
